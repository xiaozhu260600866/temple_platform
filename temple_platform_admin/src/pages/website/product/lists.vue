<template>
	<section>
		<el-col :span="24" class="toolbar t-search float-r">
			<el-form :inline="true" id="search" :model="searchFields">
				...
				<el-form-item class="mr0">                    
					<el-button type="primary" @click="$router.push({path: '/vueadmin/website/product-create'})" v-if="data.auth.canCreate">新增</el-button>
				</el-form-item>
			</el-form>
		</el-col>
		<el-table ref="multipleTable" :data="data.lists" border tooltip-effect="dark" style="width: 100%" @selection-change="selsChange" v-loading="data.listLoading">
			...
			<el-table-column prop="views" label="点击量" width="70"> </el-table-column>
			<el-table-column prop="num" label="库存" width="70"> </el-table-column>
			<el-table-column label="操作" width="170">
				<template scope="scope">
					<el-button type="primary" size="mini" @click="$router.push({path:'/vueadmin/website/product-edit?id='+scope.row.id})" v-if="data.auth.canEdit">编辑</el-button>
		...
		<my-class top="2%" ref="category" :type="0" :formAction="'/admin/website/product/'" :sizearr="300"></my-class>
...
export default {
	data() {
		return {
			formAction: '/admin/website/product/lists',
			data: this.formatData(this),
			siteName: this.getSiteName(),
			tableName: 'website_products',
			sels: [], //列表选中列,
			searchFields: {
				name: '',
			}
