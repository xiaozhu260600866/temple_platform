<template>
	<section>
		<el-col :span="24" class="toolbar t-search float-r">
			<el-form :inline="true" id="search" :model="searchFields">
				...
				<el-form-item class="mr0">
					<el-button type="primary" @click="$router.push({path: '/vueadmin/website/article-create'})" v-if="data.auth.canCreate">发布动态</el-button>
				</el-form-item>
			</el-form>
		</el-col>
		...
		<el-table ref="multipleTable" :data="data.lists" border tooltip-effect="dark" style="width: 100%" @selection-change="selsChange" v-loading="data.listLoading">
			...
			<el-table-column label="操作" width="170">
				<template scope="scope">
					<el-button type="primary" size="mini" @click="$router.push({path:'/vueadmin/website/article-edit?id='+scope.row.id})" v-if="data.auth.canEdit">编辑</el-button>
		...
		<my-class top="2%" ref="category" :type="0" :formAction="'/admin/website/article/'" :sizearr="300"></my-class>
	</section>
</template>
...
export default {
	data() {
		return {
			formAction: '/admin/website/article/lists',
