...
export default {
    mounted: function() {
        if (this.getOptions("id")) {
            this.formAction = '/admin/website/article/edit?id=' + this.getOptions("id");

        } else {
            this.formAction = '/admin/website/article/create';
        }
        this.ajax();
    },
    data() {
                ...
                if (valid) {
                    ...
                    this.postAjax(this.formAction, this.ruleForm, msg => {
                        if (msg.data.status == 2) {
                            this.$router.push({ path: '/vueadmin/website/article-lists' });
                        }
                    })
                } else {
