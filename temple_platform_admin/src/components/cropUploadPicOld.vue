<template>
	<section>
		<el-dialog title="选择图片" :visible.sync="dialogFormVisible" top="141px" :before-close="handleClose">
					...
					<div class="cropper-area">
						...
						<div class="cropper-block" :style="cropperBlockStyle">
							...
							<div class="canvas-box" v-if="originalImg">
								<canvas :width="cropperCanvasSize.width" :height="cropperCanvasSize.height" ref="cropper" :class="{'movement':movement}"></canvas>
							</div>
							<div v-else>
								<el-button @click="selectPic" type="primary">点击上传</el-button>
								<p class="tips">{{defaultTips}}</p>
							</div>
						</div>
						<el-button @click="selectPic" class="re-upload" v-if="originalImg">重新上传</el-button>
					</div>
					<!-- 预览区域 -->
  ...
  methods: {
    ...
    readImage(event) {
      ...
      image.onload = () => {
        ...
        file = { originalImgData:this.originalImgData, raw: rawfile };
