<template id="search_member">
	<div class="search_box" style="">
		<el-select v-model="value9"  filterable remote reserve-keyword placeholder="请输入关键词"
			:remote-method="remoteMethod" :loading="loading" @blur="test" @change="selectRes">
			<el-option v-for="item in options4" :key="item.id" :label="item.name" :value="item">
			</el-option>
		</el-select>
		...
		<!-- <div class="memberInfo" :style="{width:width}" v-show="memberInfoDig">
			<el-table ref="multipleTable" :data="data.lists" border tooltip-effect="dark" style="width: 100%" v-loading="data.listLoading" @row-click="selectMemberNew">
				...
				<el-table-column prop="phone2" label="电话2"> </el-table-column>
...
<script type="text/javascript">
	export default {
		...
		data() {
			return {
				searchMemberVal: '',
				formAction: '/admin/user/lists?online=0',
				memberInfo: {},
		...
		methods: {
			...
			test(){
				...
				this.value9=this.searchMemberVal
				this.$emit('callback', {username:this.value9});
			...
			remoteMethod(query) {
				...
				if (query !== '') {
					...
					this.postAjax(this.formAction, {
						...
						this.loading = false;
						this.options4 = msg.data.lists.data;
					});
...
<style type="text/css">
	...
	.search_box>.el-input .el-input-group__append {
