<template>
	<view class="poster" v-if="show">
		<image :src="receipt" @load="imageLoad" hidden v-if="url=='pages/user/order/project'"></image>
		<image :src="receipt" @load="imageLoad" hidden v-if="url!='pages/user/order/project'"></image>
		<canvas :style="'width: '+width+'px; height: 400rpx;left:-1400px'" canvas-id="firstCanvas" > </canvas>
	...
	export default {
		...
		methods: {
			...
			ajax(order,open) {
				...
				//alert(this.url)
				this.open = open;
				this.receipt = this.getSiteName() + '/upload/images/product/'+this.getParent(this).data.siteConfig.receipt_logo
			...
			imageLoad(e){
				...
				if(this.url == 'pages/user/order/project' || this.url == 'pages/user/order/shop'){
					ctx.drawImage(this.receipt, 0, 0, viewWidth, viewHeight)
				}else{
					ctx.drawImage(this.receipt, 0, 0, viewWidth, viewHeight)
				}
				...
				ctx.fillText(this.order.order_no, 278 * rpx, 49 * rpx)
				
				//姓名
				ctx.setFontSize(10)
				ctx.setFillStyle('#603217')
				ctx.fillText(this.order.bless_name, 90  * rpx, 78 * rpx)
				...
				ctx.setFontSize(10)
				ctx.setFillStyle('#603217')
				ctx.fillText(this.order.projects.name, 70 * rpx, 127 * rpx)
