...
export default {
	...
	methods: {
		...
		imageLoad(e){
			...
			let familyName = this.drawArticle(ctx, '本宅地基之界内外大小一切孤魂亡灵', 22*rpx, viewWidth/2-3,320*rpx)
			
			// // 阳上人
			// ctx.font = 'bold 20px 微软雅黑'
			// if(this.name1.length >= this.name2.length){
			// 	if(this.name3.length >= this.name4.length){
			// 		let yang = this.drawArticle(ctx, '阳上人:', 22, 25 ,viewHeight-355-this.name3.length*22-this.name1.length*22)
			// 	}else{
			// 		let yang = this.drawArticle(ctx, '阳上人:', 22, 25 ,viewHeight-355-this.name4.length*22-this.name1.length*22)
			// 	}
			// }else{
			// 	if(this.name3.length >= this.name4.length){
			// 		let yang = this.drawArticle(ctx, '阳上人:', 22, 25 ,viewHeight-355-this.name3.length*22-this.name2.length*22)
			// 	}else{
			// 		let yang = this.drawArticle(ctx, '阳上人:', 22, 25 ,viewHeight-355-this.name4.length*22-this.name2.length*22)
			// 	}
			// }
			// // 姓名
