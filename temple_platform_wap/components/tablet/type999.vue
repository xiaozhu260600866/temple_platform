	...
	methods: {
		...
		imageLoad(e){
			...
			setTimeout(()=>{
				...
				let familyName = this.drawArticle(ctx, this.familyName+'门堂上历代祖先', 27*rpx, viewWidth/2-2,370*rpx)
				
				if(this.fName){
					// 父姓
					ctx.font = 'bold 27rpx 微软雅黑'
					let fName = this.drawArticle(ctx, '先父 '+this.fName+'大人', 27*rpx, viewWidth/2+36,370*rpx)
				}
				
				if(this.mName){
					// 母姓
					ctx.font = 'bold 27rpx 微软雅黑'
					let mName = this.drawArticle(ctx, '先母 '+this.mName+'大人', 27*rpx, viewWidth/2-40,370*rpx)
				}
