...
export default {
	...
	methods: {
		...
		imageLoad(e){
			...
			let familyName = this.drawArticle(ctx,'附在身上所有杀生债主冤亲债主', 26*rpx, viewWidth/2-3,320*rpx)
			// if(this.illness.length <= 3){
			// 	ctx.font = 'bold 28px 微软雅黑'
			// 	let familyName = this.drawArticle(ctx,'造成'+this.illness+'病痛的所有冤亲债主', 25, viewWidth/2-3,320)
			// }else if(this.illness.length > 3 && this.illness.length < 6){
			// 	ctx.font = 'bold 24px 微软雅黑'
			// 	let familyName = this.drawArticle(ctx,'造成'+this.illness+'病痛的所有冤亲债主', 22, viewWidth/2-3,320)
			// }else{
			// 	ctx.font = 'bold 20px 微软雅黑'
			// 	let familyName = this.drawArticle(ctx,'造成'+this.illness+'病痛的所有冤亲债主', 18, viewWidth/2-3,315)
			// }
